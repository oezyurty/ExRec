...
class WorstKCEnv(gym.Env):
    def __init__(self, 
                question_embed_size:int=768,
        ...
        super(WorstKCEnv, self).__init__()
    ...
    def reset(self):
        ...
        if self.log_path != "":
            with open(os.path.join(self.log_path, "train_env_outs.txt"), "a+") as f:
                print(self._cluster_kc_map, file=f)
    ...
    # Using log_improvements in Train Env raises CUDA Kernal error. TODO: Investigate this error
    def log_improvements(self):
        ...
        wandb.log({
            "Train Episode End - Mean Improvement": mean_improvement.mean(),
            "Train Episode End - Max Improvement": torch.max(mean_improvement),
            "Train Episode End - Min Improvement": torch.min(mean_improvement),
            "Train Episode End - Upper Bound Improvement": max_upper_bound
        })
...
class WorstKCTestEnv(gym.Env):
    def __init__(self, 
        ...
        super(WorstKCTestEnv, self).__init__()
...
class WorstKCDiscreteEnv(gym.Env):
    def __init__(self, 
                question_embed_size:int,
                pretrained_model_path="",
                batch_size: int=1024,
        ...
        super(WorstKCDiscreteEnv, self).__init__()
        self.cluster_batch_size = cluster_batch_size
        random.seed(seed)
        np.random.seed(seed)
        torch.manual_seed(seed)
...
class WorstKCDiscreteTestEnv(gym.Env):
    def __init__(self, 
                question_embed_size:int,
                pretrained_model_path="",
                batch_size: int=1024,
        ...
        if wandb_prefix == "":
            raise ValueError("Please provide a valid wandb_prefix")
        
        super(WorstKCDiscreteTestEnv, self).__init__()
        self.cluster_batch_size = cluster_batch_size
        random.seed(seed)
        np.random.seed(seed)
        torch.manual_seed(seed)
...
class WorstKCGroundTruthEnv(gym.Env):
    def __init__(self, 
                question_embed_size:int,
                pretrained_model_path="",
                batch_size: int=1024,
        ...
        super(WorstKCGroundTruthEnv, self).__init__()
        random.seed(seed)
        np.random.seed(seed)
        torch.manual_seed(seed)
        if device == "cuda":
    ...
    def log_improvements(self):
        ...
        if self.use_ground_truth_response:
            print("Using Ground Truth Responses Results:")
        else:
            print("NOT Using Ground Truth Responses Results:")
